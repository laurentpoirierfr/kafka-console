...
func main() {
	...
	if err != nil {
		fmt.Printf("Erreur lors de la création du consommateur Kafka: %v\n", err)
		return
	}
	...
	if err != nil {
		fmt.Printf("Erreur lors de l'abonnement aux topics: %v\n", err)
		return
	}

	// Création de l'instance de Consumer
	consumerInstance := NewConsumer(consumer)

	// Démarrage de la consommation des messages avec la fonction Start
	consumerInstance.Start()

	// Ou démarrage avec une fonction de traitement de messages personnalisée
	consumerInstance.StartMessageConsumer(func(messageConsumer *KafkaMessageConsumer) {
